...
extern "C" 
{
    JNIEXPORT void JNICALL JBind_JBindWrapper_init(JNIEnv* env, jobject wrappedObject)
    {
        ...
        if(javaClass == nullptr)
        {
            ...
            << "The class inherits from JBindWrapper, however is not known to jbind.\n"
            << "In order to create new classes that are supported by jbind, it is not sufficient to simply create\n"
            << "your own class and inherit from JBindWrapper.\n"
            << "You need to add the class in C++ to a JBIND_MODULE and let jbind generate the wrapper code, not the other way round.\n"
            << "Please follow the official jbind guidelines.");
        }
