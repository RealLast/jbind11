...
namespace jbind11
{
    class JBindWrapperDeployer
    {
        private:
            ...
            {
                const std::vector<std::unique_ptr<JavaPackage>>& packages = JavaPackageManager::getInstance()->getPackages();
        ...
        public:
            ...
            {
                const std::string packageListFile = basePath + std::string("/") + std::string("JBindPackageList.txt");
                ...
                if(JBindFileUtils::fileExists(packageListFile))
                {
                    ...
                    if(!fileList.parseFrom(packageListFile))
                    {
                        ...
                        << "The file JBindPackageList.txt exists under the mentioned path, however we were unable to parse it.\n"
                        ...
                        << "files that were not generated by jbind11. Please make sure that read permissions to the file " << basePath << "/JBindPackageList.txt" << " are granted.\n"
                    ...
                    {
                        JBIND_THROW("Error in deplyoing jbind files to \"" << basePath << "\". Cannot remove existing " << basePath << "/JBindPackageList.txt")
                    }
                }
